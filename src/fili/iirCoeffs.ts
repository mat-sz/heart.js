var preCalc = function (params, coeffs) {
  ...
  if (params.BW) {
    pre.alpha =
      Math.sin(w) *
      Math.sinh(((Math.log(2) / 2) * params.BW * w) / Math.sin(w));
  } else {
    ...
    coeffs.a.push(
      -Math.pow(Math.E, s * w) *
        ...
        Math.cos(
          -w *
            Math.sqrt(
              Math.abs(Math.pow(as, 2) / (4 * Math.pow(bs, 2)) - 1 / bs)
            )
        )
    );
